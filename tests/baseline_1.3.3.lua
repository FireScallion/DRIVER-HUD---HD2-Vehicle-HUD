...
-- DRIVER HUD 1.3.3. Tank + FRV integration with isolated telemetry and bounded read-error grace.
...
if LOG_ROTATION=='same_process' then
 log('DRIVER_HUD SCRIPT_RELOAD version=1.3.3 pid='..tostring(LOG_SESSION_PID or 'unknown')..' utc='..tostring(LOG_UTC))
else
 log('DRIVER_HUD SESSION_START version=1.3.3 pid='..tostring(LOG_SESSION_PID or 'unknown')..' session='..tostring(LOG_SESSION_KEY)..' rotation='..tostring(LOG_ROTATION)..' utc='..tostring(LOG_UTC))
end
log('DRIVER_HUD 1.3.3 START native_contract=r3-cc75948d proxy_unit_health=1 wheel_fault_isolation=1 robustness_pass=1')
