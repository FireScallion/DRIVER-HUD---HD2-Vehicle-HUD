-- Focused tests run under LuaJIT 2.1. Engine APIs and memory are synthetic.
local ROOT=arg[1] or '.'
...
local function fresh()
 ...
 local exports='return {installed=true,test={M=M,Native=Native,FRV=FRV,Position=Position,C=C,update=update,clear=clear,reset=full_reset,refresh=refresh_bound}}'
 local s,n=SOURCE:gsub('return {installed=true}%s*$',exports);assert(n==1)
 local chunk=assert(loadstring(s,'@production_1_3_2.lua'));setfenv(chunk,env)
...
local function graph_fixture()
 ...
 m:table(z.net+0xF19A70,0x300000000,{{z.av,2},{z.e1,4},{z.e2,1}})
 m:table(z.net+0xF21A88,0x300001000,{{315,2},{363,4},{462,1}})
 for _,v in ipairs({{z.av,315,2},{z.e1,363,4},{z.e2,462,1}})do m:descriptor(z.net+0xF31AD8+v[3]*24,v[1],v[2])end
 m:table(z.hm+0x28,0x300002000,{{z.e1,3},{z.e2,0}})
 m:table(z.sm+0x20,0x300003000,{{z.e1,1},{z.e2,0}})
 m:table(z.seat+0x20,0x300004000,{{z.av,0}})
 m:w64(z.hm+0x40,0x300005000);m:w64(z.sm+0x38,0x300006000)
 m:w64(z.seat+0x38,0x300007000);m:w64(0x300007000,z.net+0xF31AD8+2*24)
 m:w64(z.seat+0x48,z.seatr);m:zero(z.seatr,64);m:w32(z.seatr,z.e1)
 m:w64(z.hm+0x50,z.hr);m:w64(z.sm+0x40,z.sr);m:w64(z.sm+0x50,z.rp)
 for _,v in ipairs({{z.e1,363,3,1,4},{z.e2,462,0,0,1}})do
  local dp=z.net+0xF31AD8+v[5]*24
 ...
 m:table(z.hm+0x68,0x300008000,{})
 z.st=0x800000000;m:w64(z.net+0xF11738,z.st);m:zero(z.st,984*16)
 local slot=N.mod64hex('cc21c7ffd3ebefb9',984)
...
if os.getenv('HUD_TEST_HELPERS')=='1' then return {fresh=fresh} end
...
test('native proxy resolves only unique same-unit known FRV Health owner',function()
 ...
 local e1desc=z.net+0xF31AD8+4*24;local unit=N.u32(m:read(e1desc+12,4),0)
 m:table(z.net+0xF19A70,0x300000000,{{z.av,2},{z.e1,4},{z.e2,1},{proxy,proxy_idx}})
 m:table(z.net+0xF21A88,0x300001000,{{315,2},{363,4},{462,1},{proxy_goid,proxy_idx}})
 m:descriptor(z.net+0xF31AD8+proxy_idx*24,proxy,proxy_goid,'e9cd1d0d118886af',1,unit)
 ...
 local e2desc=z.net+0xF31AD8+1*24;m:w32(e2desc+12,unit)
...
test('native dense compaction re-resolves pointer and index',function()
 ...
 z.m:table(z.hm+0x28,0x300002000,{{z.e1,1},{z.e2,0}});z.m:w64(0x300005000+8,z.net+0xF31AD8+4*24)
 local r=z:health();eq(r.hp[1],222);eq(r.health_index,1);eq(r.health_record,z.hr+440)
end)
test('native owner mismatch fails closed',function()local z=graph_fixture();z.m:w64(0x300005000+24,z.net+0xF31AD8+24);eq(pcall(function()z:health()end),false)end)
test('native reverse mapping mismatch fails closed',function()local z=graph_fixture();z.m:table(z.net+0xF21A88,0x300001000,{{315,2},{363,1},{462,4}});eq(pcall(function()z:g():relation(315)end),false)end)
...
test('native local/remote precision distinct and q2 low-byte decode',function()local z=graph_fixture();z.m:w32(z.net+0xF31AD8+4*24+20,0);z.m:w32(z.rp+16+4,0xfffffffe);local r=z:health();eq(r.precision,'QUANTIZED_OR_UNKNOWN');eq(r.q[1],2);eq(r.q[4],3)end)
...
test('native watches detect an owner change after initial read',function()local z=graph_fixture();local g=z:g();g:net(z.net,z.e1,true);z.m:w32(z.net+0xF31AD8+4*24+8,123);eq(pcall(function()g:validate()end),false)end)
test('native table non-power-two and short reads rejected',function()local z=graph_fixture();z.m:w32(z.hm+0x30,15);eq(pcall(function()z:health()end),false);local g=z.N.graph(function(p,n)return string.rep('\0',n-1)end,z.base);eq(pcall(function()g:relation(315)end),false)end)
test('native budget and noncanonical pointer rejected',function()local z=graph_fixture();local g=z:g();g.calls=640;eq(pcall(function()g:read(z.base,1)end),false);z.m:w64(z.base+z.N.roots.health,140737488355328);eq(pcall(function()z:health()end),false)end)
test('native hash multiplication does not lose low bits in double arithmetic',function()local n=fresh().a.Native;eq(n.mul32(4294967295,4294967295),1);eq(n.mul32(0xFEDCBA98,0x9E3779B1),0x7176DB18);eq(n.mod64hex('cc21c7ffd3ebefb9',984),529)end)
-- Module gate is tested against source guard bytes in a synthetic PE.
test('module PE identity plus six exact code guards required',function()
...
test('player GOID zero and HMG native seat show FRV, no numeric wheel HP',function()
 local h=fresh();h:enter(h:add_frv(336,467));h:tick(1);eq(h.a.M.player,0);eq(h.a.FRV.mode,'FRV');eq(h.a.FRV.vehicle.entity,467);eq(#h:texts(),1);eq(h:texts()[1].args[2],'2400');h:clean()
end)
...
test('tank to FRV to tank clears shared GUI and restores ammunition',function()
 local h=fresh();local tank=h:add_tank(351,600);local frv=h:add_frv(336,467)
...
test('health failure expires bars but retains valid body-only FRV',function()
 local h=fresh();h:enter(h:add_frv(336,467));h:tick(.3);h.health_fail=true;h:tick(.6);eq(h.a.FRV.data,nil);eq(h.a.FRV.hp,2400);eq(#h:texts(),1);eq(h.a.M.hull,nil);h:clean()
end)
...
test('body contour and number thresholds include exactly 75 and 50 percent',function()
 ...
 local h=fresh();local d=h:add_frv(336,467);h.objects[336].hp=1800;h:enter(d);h:tick(.3);local t=h:texts()[1];eq(t.args[7][3],205)
...
test('body number uses min+max bounds for exact x/y centering',function()
 local h=fresh();h:enter(h:add_frv(336,467));h:tick(.3);local t=h:texts()[1].args;local x,y,s=h.a.FRV.layout(1920,1080)
...
test('Vector2 accessor-only text extents stay on the native font path',function()
 local h=fresh();h.env.stingray.Gui.text_extents=function(_,label,font,size)return {-1,-3},{#label*size*.53-2,size*.75}end
...
print(string.format('RESULT HUD_1_3_2_WHEEL_FAULT_ISOLATION cases=%d fails=%d',count,failed))
